...
const farmLocate = () => {
  ...
  let lettrLen = orderItem.value;
  if (lettrLen != "Mattuga" && lettrLen != "Maganjo") {
    orderItem.focus();
    orderItem.style.border = "2px solid red";
...
const purchaseStatus = () => {
  let orderItem = document.purchaseorder.status;

  let lettrLen = orderItem.value;
  if (lettrLen != "PENDING" && lettrLen != "RECEIVED") {
    orderItem.focus();
    orderItem.style.border = "2px solid red";
    // alert('Location Must Be Between 10 and 30 Characters!');
    return false;
...
const supplierCateg = () => {
  ...
  let lettrLen = suppCat.value;
  if (lettrLen != "Individual" && lettrLen != "Enterprise") {
    suppCat.focus();
    suppCat.style.border = "2px solid red";
    //   alert('Select a Category from the list!');
    return false;
